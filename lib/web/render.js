  ...
  const tableRows = rows
    .map(
      (r) => `<tr>
        ...
        <td>${escapeHtml(r.scores?.revenue ?? '-')} / ${escapeHtml(r.scores?.attraction ?? '-')} / ${escapeHtml(r.scores?.retention ?? '-')}</td>
        <td>${escapeHtml(r.scores?.effort ?? '-')}</td>
...
<form method="get" action="/ideas">
  <label>Workspace <select name="repo"><option value="">(all)</option>${repoOptions}</select></label>
  <label>Status <input type="text" name="status" value="${escapeHtml(filters.status ?? '')}" placeholder="e.g. proposed"></label>
  <button type="submit">Filter</button>
</form>
<table>
  <thead><tr><th>Workspace</th><th>Title</th><th>Status</th><th>Rev/Attr/Ret</th><th>Effort</th><th>Updated</th></tr></thead>
  <tbody>${tableRows || '<tr><td colspan="6">No ideas found.</td></tr>'}</tbody>
</table>`;
}
...
<dl class="field">
  ...
  <dt>Workspace</dt><dd>${escapeHtml(repoAlias)} (${escapeHtml(idea.workspace_kind ?? '')}: ${escapeHtml(idea.workspace_path ?? '')})</dd>
  <dt>Repos touched</dt><dd>${(idea.repos ?? []).length ? escapeHtml(idea.repos.join(', ')) : '<span class="muted">none recorded</span>'}</dd>
  ...
  const outcomeEligible = [
    'implemented', 'changes_requested', 'approved_for_merge', 'partially_merged',
    'merged', 'reverted', 'closed', 'implementation_failed', 'merge_failed'
  ];
  const outcomeBlock = outcomeEligible.includes(idea.status)
    ? `<form method="post" action="${base}/outcome">
    ${csrfField(csrfToken)}
    <fieldset>
      <legend>Record outcome</legend>
      <label>Window
        <select name="window">
      ...
      <div class="field">
        <label for="outcome-metrics">Metrics (one key=value per line, e.g. dissent_was_correct=true)</label><br>
        <textarea id="outcome-metrics" name="metrics" rows="3" cols="60" placeholder="dissent_was_correct=true" required></textarea>
      </div>
      <div class="field">
        <label for="outcome-note">Note (the actual reasoning/evidence)</label><br>
        <textarea id="outcome-note" name="note" rows="3" cols="60"></textarea>
      </div>
