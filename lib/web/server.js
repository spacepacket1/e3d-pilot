  ...
  try {
    switch (action) {
      ...
      case 'sync':
        ...
        break;
      case 'outcome': {
        const window = form.get('window') || 'custom';
        const metrics = (form.get('metrics') || '')
          .split('\n')
          .map((line) => line.trim())
          .filter(Boolean);
        if (metrics.length === 0 || metrics.some((m) => !m.includes('='))) {
          return handleIdeaDetail(req, res, aliasToWorkspace, alias, id, {
            error: 'Outcome needs at least one metric line in key=value form.'
          });
        }
        recordOutcome(ws, id, { window, metrics, note: form.get('note') || '' });
        break;
      }
      case 'implement':
        implementIdea(ws, id);
  ...
  return async function requestListener(req, res) {
    try {
      ...
      if (
        ...
        segments[0] === 'ideas' &&
        segments.length === 4 &&
        ['approve', 'reject', 'request-changes', 'approve-merge', 'sync', 'outcome', 'implement'].includes(segments[3])
      ) {
