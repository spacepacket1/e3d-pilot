---
run_id: impl-9b2cd6b0750a-repo-20260909181616
---
...
- Build a deterministic JSON snapshot from `events.jsonl` and materialized `idea.json` files.
- Send that snapshot with one authenticated HTTP POST to a repository-agnostic URL.
...
- Obtain the API key only from the environment-variable name in configuration. Never store its value in config, the payload, artifacts, or diagnostic output.
- Use existing Bash, `jq`, and `curl` dependencies; add no runtime or package dependency.
- Make exactly one request per mirror invocation, with no retry and no redirect following so credentials cannot be forwarded to another host.
...
- Explicit `storage mirror` execution is strict: invalid configuration, invalid local JSON, a missing credential, curl failure, or non-success HTTP status must produce a nonzero exit.
...
1. Extend `config.schema.json` with an optional strict `storage.mirror` object containing exactly:
   ...
   - `api_key_env`: required environment-variable name matching a portable shell identifier such as `^[A-Za-z_][A-Za-z0-9_]*$`.

   Reject unknown properties in `storage` and `storage.mirror` consistently with the schema’s existing strictness. Do not require `storage` or `mirror`, so all existing valid configurations remain valid.
...
2. Add `lib/ideas/mirror.sh` following the repository’s existing shell-library conventions. Expose narrowly scoped functions for:
   ...
   - Building the current snapshot.
   - Posting the snapshot.
   - Running the mirror in strict or best-effort mode.

3. Treat an absent `storage.mirror` block or `"enabled": false` as disabled. Disabled automatic use must return successfully without inspecting the named credential or invoking curl.

4. For an enabled mirror, validate before any network request that:

   - The URL has an `http` or `https` scheme, a non-empty authority, no user information, and no fragment.
   - `api_key_env` is a valid environment-variable name.
   - The named environment variable exists and has a non-empty value.
   - `.e3d-pilot/events.jsonl` exists or can be represented as an empty event stream.
   - Every nonblank event line is valid JSON.
   - Every discovered `.e3d-pilot/ideas/*/idea.json` file is valid JSON.

5. Build one JSON request body with this stable top-level contract:

   - `schema_version`: integer `1`.
   - `repository`: an object containing a stable repository identifier derived from the credential-scrubbed `origin` URL when available, otherwise the repository directory basename.
   - `mirrored_at`: a UTC timestamp.
   - `events`: an array preserving nonblank `events.jsonl` records in file order.
   - `ideas`: an array of materialized `idea.json` objects ordered lexically by idea directory.

   Do not include local absolute paths, the API key, environment contents, evidence file contents, run artifacts, or source files.

6. Send the payload using exactly one curl request:

   - HTTP `POST` to the configured URL.
   - `Content-Type: application/json`.
   - `Authorization: Bearer <resolved key>`.
   - Request body supplied without embedding it in a shell command string.
   - No redirect following and no retry.
   - Bounded connection and total request time.
   - HTTP non-success status treated as failure.
   - Response body discarded rather than copied into ledger or run state.

7. Use a temporary snapshot with restrictive permissions where needed and remove it on success, error, or interruption. Do not create persistent mirror state beneath `.e3d-pilot`.

8. Add `tests/phase34.sh` using temporary repositories and a fake `curl` placed on `PATH`; tests must not require network access or real credentials. Cover schema acceptance and rejection, deterministic event/idea ordering, repository identifier credential scrubbing, request method and headers, one-request behavior, timeout/retry/redirect options, disabled no-op behavior, missing credentials, malformed ledger input, HTTP/curl failure, temporary-file cleanup, and absence of secret values from payload and diagnostics.

### Acceptance Criteria

- Existing configurations with no `storage` field still validate and make no mirror request.
- A valid disabled or enabled mirror block passes schema validation; malformed URLs, invalid environment-variable names, missing required keys, wrong types, and unknown mirror keys fail validation.
...
- The fake transport confirms redirects and retries are disabled and request time is bounded.
- No API-key value appears in captured request bodies, stdout, stderr, or persistent files.
...
2. Add the on-demand command:
   ...
   Integrate it with existing help, repository resolution, configuration loading, and argument-error conventions. Reject unknown storage subcommands and unexpected arguments.
...
3. Run the on-demand command in strict mode:
   ...
   - Configuration, snapshot, transport, and HTTP failures print an actionable error and exit nonzero.

4. Invoke best-effort mirroring at the central point after a publish backend has successfully completed and its required publication state has been recorded. The placement must cover ordinary implementation, `repair-run` publication resumes, and each successfully published fleet target without duplicating a request within one publish attempt.
...
6. If automatic mirroring fails after successful publication, emit one sanitized warning and retain the publish command’s successful status and artifacts. Do not retry publication, roll it back, append an idea event, or report the idea as implementation-failed solely because mirroring failed.
...
7. Extend `tests/phase34.sh` to exercise CLI parsing and the publish hook with faked transport and publication dependencies. Verify:
   ...
   - Help and invalid-subcommand behavior.
   - No request for absent or disabled configuration.
   - One request after successful publication.
   - No request after failed publication.
   - Automatic mirror failure leaves successful publication successful.
   - The same central hook is reached by resumed publication and fleet target publication paths.
   - Existing idea lifecycle state and event counts are unchanged by every mirror outcome.
...
9. Update `README.md` to document:
   ...
   - The versioned payload contents.
   - Automatic post-publish behavior and best-effort failure semantics.
   ...
   - That enabling the feature for a repository is a separate manual edit to that repository’s `.e3d-pilot/config.json`.
...
- Every successful publish path attempts at most one enabled mirror upload after publication.
- Failed or skipped publication never invokes the mirror.
- An automatic mirror failure produces a sanitized warning while the already-successful publish operation remains successful.
...
Reason: Spec is internally consistent, scoped within stated file/line budget, and phases have clear, testable, non-contradictory requirements.
...
Reason: The draft conflicts on deterministic snapshots versus a variable timestamp and leaves repository-ID sanitization and secret-safe transport testing underspecified.
...
5. Build one JSON request body with this stable top-level contract:
   ...
   - For `http://`, `https://`, `ssh://`, or other `scheme://` origins, remove user information, query, and fragment components while retaining the scheme, host, optional port, and path.
...
7. Use a temporary payload file with mode `0600` where a file is needed and remove it on success, validation error, curl error, HTTP error, or handled interruption. Do not create persistent mirror state beneath `.e3d-pilot`.

8. Add `tests/phase34.sh` using temporary repositories and a fake `curl` placed on `PATH`; tests must not require network access or real credentials. Cover schema acceptance and rejection, event and idea ordering, timestamp shape, repository-identifier fallback and credential/query/fragment scrubbing, request method and headers, one-request behavior, timeout/retry/redirect options, disabled no-op behavior, missing credentials, malformed ledger input, HTTP/curl failure, temporary-file cleanup, and absence of secret values from payload and diagnostics.

   The fake curl must verify the Authorization argument in-process against the expected value and record only a boolean or redacted result. It must not persist the raw key or complete Authorization header in an argument log. Any temporary request-body capture must be checked for secret absence and removed by the test.
...
- No API-key value appears in captured request bodies, stdout, stderr, production artifacts, or retained test files.
- `set -e; for test_file in tests/phase*.sh; do bash "$test_file"; done` passes.
...
9. Update `README.md` to document:
   ...
   - The versioned payload contents, ordering, changing timestamp, and repository-identifier sanitization.
...
Reason: Authorization header handling doesn't match the spec's own secret-handling bar—passing the key via a curl -H command-line argument would expose it in process listings (ps/proc) during the request, undermining the "must not print the API-key value" constraint.
...
8. Add `tests/phase34.sh` using temporary repositories and a fake `curl` placed on `PATH`; tests must not require network access or real credentials. Cover schema acceptance and rejection, event and idea ordering, timestamp shape, repository-identifier fallback and credential/query/fragment scrubbing, request method and headers, one-request behavior, timeout/retry/redirect options, disabled no-op behavior, missing credentials, malformed ledger input, HTTP/curl failure, temporary-file cleanup, and absence of secret values from payload, diagnostics, and process arguments.
...
Reason: Repository-identifier rules conflict for file-style or authority-less scheme origins, which would retain a local absolute path despite explicitly prohibiting it.
...
Reason: Spec is internally consistent, addresses credential/transport security thoroughly, and has clear testable acceptance criteria within stated file/line budgets.
...
Reason: The draft is internally consistent, implementation-ready, security-conscious, and provides complete requirements and acceptance coverage within the stated scope.
