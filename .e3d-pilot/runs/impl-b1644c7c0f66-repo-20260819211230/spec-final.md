...
- Keep the complete change within five files, approximately 250 changed lines of implementation code (the test file may reasonably run longer given this repo's fake-executable-on-PATH stubbing convention), and never exceed 15 changed files or 900 changed lines total.
...
- All new/edited shell scripts must run correctly under Bash 3.2 (macOS's default `/bin/bash`), which this repo's tests execute under. Do not use `declare -g`/`declare -ag` (no `-g` flag support), `${var,,}`/`${var^^}` case conversion, or any other Bash 4+-only construct. Lowercase strings with `tr '[:upper:]' '[:lower:]'` instead of `${var,,}`. Declare "global" arrays as plain top-level `array_name=()` (top-level assignments are already global in Bash 3.2) instead of `declare -g`/`declare -ag`. Guard every `"${arr[@]}"` expansion against an empty/unset array under `set -u` using the `"${arr[@]+"${arr[@]}"}"` idiom.
...
<!-- pilot:touches=tests/phase26.sh -->
...
16. Add `tests/phase26.sh` using the established fake-executable-on-`PATH` convention. All files created by the test, including fake curl state and trace files, must remain inside a test-owned temporary directory removed by a trap. Tests must not contact the network.
...
- `tests/phase26.sh` supplies a fake `curl` earlier on `PATH`, records every invocation, and fails if the collector bypasses that stub.
...
- The final diff touches no more than the five declared files, remains below 900 changed lines, and modifies no protected path.
